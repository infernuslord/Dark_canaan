...
#endif
{
    ...
    else
    {
        ...
        _itoa((pTm->tm_yday << 11) | (pTm->tm_min + (pTm->tm_hour * 60)), szGeneratedFileName + 3, 16);
    ...
    m_RecFile = _open(m_RecFileSpec.GetName(), access, S_IREAD | S_IWRITE);
...
{
    _write(m_RecFile, &type, sizeof(type));
    _write(m_RecFile, &dataInfo, sizeof(dataInfo));
    return _write(m_RecFile, pData, dataSize) == dataSize;
}
...
{
    return _read(m_RecFile, pHeader, sizeof(sRecBlockHeader)) == sizeof(sRecBlockHeader);
}
...
{
    return _read(m_RecFile, pReadTo, size) == size;
}
...
STDMETHODIMP cRecorder::AddToStream(void *pData, size_t sizeData, const char *pszTypeTag)
{
    ...
    if (m_fFirstAccess)
    {
        ...
        _write(m_RecFile, recHeaderStr.operator const char *(), recHeaderStr.GetLength() + 1);
        _write(m_RecFile, &kRecVersion, sizeof(kRecVersion));
...
STDMETHODIMP cRecorder::ExtractFromStream(void *pData, size_t sizeData, const char *pszTypeTag)
{
    ...
    if (m_fFirstAccess)
    {
        ...
        _read(m_RecFile, &version, sizeof(version));
