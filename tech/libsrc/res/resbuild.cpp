...
//  If compound, write out reftable without compression
   ...
   if (prd2->flags & RDF_COMPOUND)
   {
      ...
      old_retval = retval;
      retval += _write(prf->fd, p, sizeTable);
...
//  If compression, try it (may not work out)
   ...
   if (pDirEntry->flags & (RDF_LZW | RDF_PKZIP))
   {
      ...
      else
      {
         ...
         old_retval = retval;
         retval += _write(prf->fd, pcompbuff, compsize);
...
//  Pad to align on data boundary
   ...
   {
      old_retval = retval;
      retval += _write(prf->fd, pad, padBytes);
...
static void ResCopyBytes(int fd, long writePos, long readPos, long size)
{
   ...
   while (size > 0)
   {
      ...
      _lseek(fd, readPos, SEEK_SET);
      _read(fd, buff, sizeCopy);
      _lseek(fd, writePos, SEEK_SET);
      _write(fd, buff, sizeCopy);
      readPos += sizeCopy;
      writePos += sizeCopy;
      size -= sizeCopy;
   }
