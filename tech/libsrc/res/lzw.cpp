...
static unsigned int LzwInputCodeFileToMemory()
{
   ...
   while (lzwe.lzwInputBitCount <= 24)
   {
      ...
      {
         //LZWTimerStop(0);
         _read(lzwFdSrc, lzwFdReadBuff, LZW_FD_TO_MEM_READ_BUFF_SIZE);
         //LZWTimerStart();
         lzwReadBuffIndex = 0;
      }
   }
...
uchar LzwFdSrcGet()
{
    ...
    {
        //LZWTimerStop(0);
        _read(lzwFdSrc, lzwFdReadBuff, LZW_FD_READ_BUFF_SIZE);
        //LZWTimerStart();
        lzwReadBuffIndex = 0;
    }
...
void LzwFdDestCtrl(long destLoc, LzwCtrl ctrl)
{
    ...
    else if (ctrl == END)
    {
        if (lzwWriteBuffIndex)
            _write(lzwFdDest, lzwFdWriteBuff, lzwWriteBuffIndex);
    }
}
...
void LzwFdDestPut(uchar b)
{
    ...
    {
        _write(lzwFdDest, lzwFdWriteBuff, LZW_FD_WRITE_BUFF_SIZE);
        lzwWriteBuffIndex = 0;
    }
}
//  ---------------------------------------------------------------
