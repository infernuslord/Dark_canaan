...
//  drop thru to create case by faking mode 3.
   ...
   if (mode != ROM_CREATE)
   {
      ...
      if (gSpoofDrive.callback && (!_stricmp(DatapathLastPath(&gDatapath), gSpoofDrive.path)))
         cd_spoof = TRUE;
      if (fd >= 0)
      {
         _read(fd, &fileHead, sizeof(ResFileHeader));
...
//  If create mode, or edit/create failed, try to open file for creation.
   ...
   if (mode == ROM_CREATE)
   {
      fd = _open(fname, O_CREAT | O_TRUNC | O_RDWR | O_BINARY,
                S_IREAD | S_IWRITE);
      if (fd < 0)
      {
...
//  if no edit info then process piecemeal.
      ...
      case ROM_EDITCREATE:
         ...
         {
            _lseek(fd, fileHead.dirOffset, SEEK_SET);
            _read(fd, &dirHead, sizeof(ResDirHeader));
...
//  If reached end of local directory buffer, refill it
      ...
      {
         _read(fd, dirEntries, sizeof(ResDirEntry) * NUM_DIRENTRY_BLOCK);
...
//  If loadonopen flag set, load resource
   ...
   {
      currOffset = _tell(resFile[filenum].fd);
      ResLoadResource(pDirEntry->id);
      ResCacheAdd(pDirEntry->id);
      _lseek(resFile[filenum].fd, currOffset, SEEK_SET);
   }
}
//  --------------------------------------------------------------
//
...
//  Seek to start of file, read in header
   ...
   _lseek(prf->fd, 0L, SEEK_SET);
   _read(prf->fd, &pedit->hdr, sizeof(pedit->hdr));
...
//  Read directory header
   ...
   pedit = prf->pedit;
   phead = &pedit->hdr;
   _lseek(prf->fd, phead->dirOffset, SEEK_SET);
   _read(prf->fd, &dirHead, sizeof(ResDirHeader));
...
//  Read in directory into allocated space (past header)
   ...
   _read(prf->fd, RESFILE_DIRENTRY(pdir, 0),
...
//  Seek to current data location
   ...
   _lseek(prf->fd, pedit->currDataOffset, SEEK_SET);
}
//  --------------------------------------------------------------
//
...
void ResWriteDir(int filenum)
{
   ...
   prf = &resFile[filenum];
   _lseek(prf->fd, prf->pedit->currDataOffset, SEEK_SET);
   _write(prf->fd, prf->pedit->pdir, sizeof(ResDirHeader) +
...
void ResWriteHeader(int filenum)
{
   ...
   _lseek(prf->fd, 0L, SEEK_SET);
   _write(prf->fd, &prf->pedit->hdr, sizeof(ResFileHeader));
}
