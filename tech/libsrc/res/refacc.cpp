...
//  Seek to data, read numrefs, allocate table, read in offsets
   ...
   _lseek(fd, RES_OFFSET_DESC2REAL(prd->offset), SEEK_SET);
   _read(fd, &numRefs, sizeof(RefIndex));
   prt = (RefTable *) Malloc(REFTABLESIZE(numRefs));
   prt->numRefs = numRefs;
   _read(fd, &prt->offset[0], sizeof(long) * (numRefs + 1));
...
//  Seek to data, read numrefs, check table size, read in offsets
   ...
   _lseek(fd, RES_OFFSET_DESC2REAL(prd->offset), SEEK_SET);
   _read(fd, &prt->numRefs, sizeof(RefIndex));
   ...
   _read(fd, &prt->offset[0], sizeof(long) * (prt->numRefs + 1));
...
//  Check id and file number and make sure compound
   ...
   else
   {
      ...
      _lseek(fd, RES_OFFSET_DESC2REAL(prd->offset), SEEK_SET);
      _read(fd, &result, sizeof(RefIndex));
      ResThreadUnlock();
      return result;
   }
}
...
//  Check id, get file number
   ...
   else
   {
      ...
      _lseek(fd, RES_OFFSET_DESC2REAL(prd->offset), SEEK_SET);
      _read(fd, &numrefs, sizeof(RefIndex));
      _lseek(fd, index * sizeof(long), SEEK_CUR);
      _read(fd, &offset, sizeof(long));
      _read(fd, &refsize, sizeof(long));
      refsize -= offset;
...
//  Seek to start of all data in compound resource
   ...
   _lseek(fd, RES_OFFSET_DESC2REAL(prd->offset) + REFTABLESIZE(numrefs),
         SEEK_SET);
...
//  If LZW, extract with skipping, else seek & read
   ...
   {
      _lseek(fd, offset - REFTABLESIZE(numrefs), SEEK_CUR);
      _read(fd, buff, refsize);
   }
