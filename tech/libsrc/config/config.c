...
{
   char* s;
   for (s = buf; s < buf + bufsiz - 1 && !_eof(fd); s++)
   {
      _read(fd,s,1);
      if (*s == '\n')
         break;
...
{
   return _strnicmp(var1,var2,VARNAME_LEN);
}
...
void config_write_line(int fd, const char* var, char* val, char* com)
{
   ...
   strcat(buf,"\n");
   if (_write(fd,buf,strlen(buf))!=strlen(buf))
...
errtype config_write_file(const char* in,writefunc writable)
{
   ...
   strcat(fn,"_");
   strcat(fn,fbuf+1);
   ofd = _open(fn,O_WRONLY|O_CREAT|O_TRUNC,S_IWRITE);
   if (ofd == -1) return ERR_FOPEN;
   config_write_iter_success=TRUE;

   ifd = _open(in,O_RDONLY);
   if (ifd != -1)
   {
      while(!_eof(ifd))
      {
   ...
   for (i = 0; i < cnt; i++)
   {
      switch(type)
      {
         ...
         case CONFIG_INT_TYPE:
            _itoa(((int*)fillvec)[i],s,10);
            break;
         case CONFIG_FLOAT_TYPE:
            _gcvt(((float *)fillvec)[i],10,s);
            break;
      }
      if (i < cnt-1)
      {
   ...
   switch(type)
      {
         case CONFIG_STRING_TYPE:
            ...
            break;
         case CONFIG_INT_TYPE:
            _itoa((int)value,s,10);
            break;
         case CONFIG_FLOAT_TYPE:
            _gcvt(*(float *)(&value),10,s);
            break;
